...
export function currentUid(): number {
  ...
  if (uid === undefined) throw new Error("subagent-watch kräver ett POSIX-system.");
  return uid;
}
...
/**
 * Appends data to a private file unless the file would grow past maxBytes. Returns false
 * when it would. The data is written with a single call, so lines from concurrent hooks
 * never interleave.
 */
export function appendPrivateFileLimited(path: string, data: Uint8Array, maxBytes: number, uid = currentUid()): boolean {
  const fd = openNoFollow(path, constants.O_WRONLY | constants.O_APPEND | constants.O_CREAT, 0o600);
  try {
    const st = assertOwnedFile(fd, path, uid, true);
    if (st.size + data.length > maxBytes) return false;
    writeAll(fd, data);
    return true;
  } finally {
    closeSync(fd);
